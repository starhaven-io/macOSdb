...
### Rescanning a published release

After a scanner fix, dispatch `Rescan Release` from `main` with the product, version, and build of each affected release. The hosted prepare job reads the release date, prerelease flags, device flag, and source URL from the published detail on `main`, so a rescan reproduces the published metadata and changes only scanner output. The self-hosted job never downloads: it requires the cached archive and its checksum sidecar as regular files, verifies the checksum and the release-date modification time, reuses a saved AEA key when present, and writes nothing to the archive drive. The publisher verifies the artifact in replace mode, which accepts only a change to that one release, and opens a `fix(data): rescan` pull request. A rescan that reproduces the published data opens no pull request. Publication branches are unique to each dispatch. A retry reuses its branch or pull request only when the signed commit has the recorded base as its sole parent and exactly matches the verified artifact overlay; unexpected state stops publication. A closed, unmerged pull request requires a fresh dispatch. If overlaying the index fails, replace mode restores the original detail bytes. Metadata corrections such as promoting a release candidate are ordinary reviewed data pull requests.
...
Changes under `site/` or `data/`, or to the deployment workflow and its validation scripts, deploy from `main` to Cloudflare Workers. Manual production dispatches fail outside `main`. Rejected dispatches use separate concurrency groups so they cannot cancel production work. CI checks the install-script policy before a clean dependency install, then runs formatting and type checks, unit tests, production build, and deployment dry-run. The production workflow revalidates the release corpus and repeats the clean install and build in a job without deployment credentials. A separate `cloudflare` environment job installs dependencies without building, downloads the build output, and deploys it with `site/scripts/deploy.mjs`. That script derives the Wrangler configuration from the checked-in `site/wrangler.json`, ignores the generated build configuration, and rejects non-regular files in the build output. The workflow verifies the live security headers after publishing.
